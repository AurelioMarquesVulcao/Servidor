...
if (cluster.isMaster) {
  ...
  cluster.on("exit", (worker, code, signal) => {
    console.log(
      `Worker ${worker.process.pid} morreu. Iniciando um novo worker...`
    );
    cluster.fork();
  });
} else {
  ...
  async function processImageTask() {
    ...
    try {
      const task = await client.lPop("fila-de-extracao"); // rPop retorna o valor de forma assíncrona
      ...
      if (task) {
        ...
        try {
          ...
          console.error(
            `Falha ao processar a tarefa ${titulo} - ${ordenador}:`,
            error
          );
          ...
          console.log(
            `Tarefa ${titulo} - ${ordenador} recolocada na fila devido a falha.`
          );
        }
      } else {
