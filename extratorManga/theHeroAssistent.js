...
const titulo = "the-hero-became-an-academy-assistant-professor-after-a-divorce"; // Título do mangá ou da página
const inicio = 1; // Número inicial do capítulo
const fim = 38; // Número final do capítulo
...
const extractMultipleChapters = async (inicio, fim) => {
  ...
  for (let capitulo = inicio; capitulo <= fim; capitulo++) {
    const capituloStr = capitulo.toString();
    ...
    const url = `https://hentaiteca.net/obra/${titulo}/capitulo-${capituloStr}/`;
    console.log(url);
